...
@section('title', 'Line Item Report')
@section('content_header')
<!-- CSRF Token -->
<meta name="csrf-token" content="{{ csrf_token() }}">
<h1 class="header-text">
    @if(isset($search))
    <a href=" {{ route('thirdparty_orders') }} " >CREATE WAREHOUSE ORDER</a>
...
<div class="box box-solid box-primary spectrumbox-primary">
  ...
  <div class="form-group col-md-6" >
    <div class="form-group"></br>
        ...
        <input type="text" class="form-control tpo-dates datepicker" name="from_date" placeholder="From Date" value="{{$from_date}}" required>
    </div>
   </div>
   <div class="form-group col-md-6" >
    <div class="form-group"></br>
        <label for="total_weight">To Date</label>
        <input type="text" class="form-control tpo-dates datepicker" name="to_date" placeholder="To Date" value="{{$to_date}}"  required>
    </div>
   </div>
  </div>
    ...
    <div class="box-body">
            ...
            <thead class="table_head">
                <tr>
                    ...
                    <th class="table_head_th">Carrier</th>
                    <th class="table_head_th">Carrier Method</th>
                    <th class="table_head_th">Shipping Cost</th>
                </tr>
            </thead>
            <tbody>
                @foreach(array_slice($reportData, 5) as $key => $value)
                    <tr>
                        @foreach($value as $k=>$v)
                            ...
                            @endif
                                @if($k == 4)
                                <a href="/thirdparty/orders/details/{{$v}}" target="_blank">
                                @endif
                                {{$v}}
                                @if($k == 4)
                                </a>
