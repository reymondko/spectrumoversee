        ...
        **/
        ...
        searchTransactionId(){
            if(this.transaction_id_from_input != null) {
                ...
                //clean up the input if long tracking number from USPS or Fedex
                if (this.transaction_id_from_input.length > 30 && this.transaction_id_from_input.substring(0,2) == '42') {//usps
                  this.transaction_id_from_input = this.transaction_id_from_input.substring(this.transaction_id_from_input.length-22);
                } else if(this.transaction_id_from_input.length > 32) { //fedex
                  this.transaction_id_from_input = this.transaction_id_from_input.substring(this.transaction_id_from_input.length-12);
                }
                ...
                axios.post('/quality-inspector/search', {
                    ...
                    if(status == 'ok') {
                        ...
                        ref.customer_id = order_details.CustomerID;
                        ref.show_search_loader = false;
                        ref.order_items = data.order_items;
                        ref.order = {
                            name:data.order_details.CustomerDescription,
                            address1:data.order_address.AdressText,
                            address2:data.order_address.Description,
                            city:data.order_address.CityDescription,
                            state:data.order_address.StateDescription,
                            zip:data.order_address.PostalCodeDescription,
                            country:data.order_address.CountryDescription,
                            company: (data.order_details.CompanyName !== "" ? data.order_details.CompanyName:order_details.CustomerDescription),
                            ref_number:data.order_details.Code,
                            phoneNumber:data.order_address.Phone,
                            emailAddress:data.order_address.Email,
                        };

                        ref.routingInfo = {
                            courier:data.order_details.CarrierDescription,
                            trackingNumber:data.order_details.CarrierTrackingNumber,
                        }
                        //ref.getSubkitIDs();
